...
export function NodeItem({
  ...
  const isHover = useHover(nodeRef, {
    // onChange(isHovering) {
    //   onChangeHover && onChangeHover(isHovering, node.nid);
    // },
  });
  const [forceOpen, setOpen] = useState<boolean>(false);
  useClickAway(() => {
    setOpen(false);
  }, [nodeRef, contentRef]);
  ...
  return (
    <span>
      <Popover
        content={forceOpen ? <Content ref={contentRef} uid={uid} /> : "点击查看详情"}
        placement="bottomLeft"
        arrow={false}
        open={forceOpen || isHover}
      >
        <span
          ref={nodeRef}
          data-node-nid={uid}
          ...
          `}
          style={{ borderColor: isHover ? "#17BA4F" : "#e6e9f0" }}
        >
...
function InternalContent({ uid }: { uid: string }, ref: ForwardedRef<HTMLSpanElement>) {
  ...
  return (
    <span ref={ref}>
      content {uid} <button onClick={() => setCount((c) => c + 1)}>{count}</button>
      {count % 2 === 0 ? "loading" : new Array(count).fill(null).map((_, index) => <p key={index}>{index}</p>)}
    </span>
  );
}
const Content = forwardRef(InternalContent);
