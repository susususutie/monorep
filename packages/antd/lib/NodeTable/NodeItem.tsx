...
export function NodeItem(props: NodeItemProps) {
  const { uid, isBig, data, row, col, rowSize, colSize, onClick, isActive } = props;

  const nodeRef = useRef(null);
  const isHover = useHover(nodeRef);

  const imgSize = isBig ? 20 : 30;
  const img = [imgGreen, imgRed][data.status] || imgRed;

  const [collapse, setCollapse] = useState(true);

  return (
    <div role="node-root" className={isBig ? cls.root : undefined}>
      <Popover content={isBig ? undefined : `点击查看详情`} placement="bottomLeft" arrow={false}>
        <span
          ref={nodeRef}
          data-node-nid={uid}
          onClick={() => {
          ...
          className={isBig ? cls.leftIcon : cls.topIcon}
          style={{ borderColor: isHover || isActive ? "#17BA4F" : "#e6e9f0" }}
        >
          <img width={imgSize} height={imgSize} src={img} />
        </span>
      </Popover>
      <span className={isBig ? cls.topLabel : cls.bottomLabel}>
        {data?.name}
      ...
      {isBig && (
        <>
          <div className={cls.info}>
            <span className={cls.label}>主机IP</span>
            <span className={cls.value}>192.168.1.1</span>
          </div>
          <div className={cls.info}>
            <span className={cls.label}>访问IP</span>
            <span className={cls.value}>192.168.1.1</span>
          </div>
          {!collapse &&
            ["虚拟IP", "虚拟IP (访问)", "网卡别名", "资产状态"].map((item) => (
              <div className={cls.info} key={item}>
                <span className={cls.label}>{item}</span>
                <span className={cls.value}>192.168.1.1</span>
              </div>
            ))}
          <div className={cls.collapseWrapper}>
            <span className={cls.collapseIcon} onClick={() => setCollapse(!collapse)}>
              {collapse ? "v" : "^"}
            </span>
          </div>
        </>
